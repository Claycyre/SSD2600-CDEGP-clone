...
) : Controller
{
    public async Task<IActionResult> Index()
    {
        ...
        var orderGroups = orders
            .Select(o => new OrderGroupViewModel
            {
                ...
                Items = o
                    .OrderLineItems.Select(li => new OrderItemViewModel
                    {
                        Id = li.FkProductSKU,
    ...
    private static string GetStatusTimeframe(string status) =>
        status switch
        {
    ...
    private static string BuildSpecifications(Product? product)
    {
        ...
        List<string> parts = [];
        if (product.AtomicNumber.HasValue)
            parts.Add($"Atomic Number: {product.AtomicNumber}");
        if (!string.IsNullOrEmpty(product.StateOfMatter))
            parts.Add($"State: {product.StateOfMatter}");
        if (!string.IsNullOrEmpty(product.ProductType))
            parts.Add($"Type: {product.ProductType}");
        if (!string.IsNullOrEmpty(product.HalfLife))
            parts.Add($"Half-Life: {product.HalfLife}");
        if (!string.IsNullOrEmpty(product.Purity))
            parts.Add($"Purity: {product.Purity}");
        if (!string.IsNullOrEmpty(product.SpecificActivity))
            parts.Add($"Specific Activity: {product.SpecificActivity}");
        return string.Join("\n", parts);
    }
}
